...
│
├── Bank Credit Card Analysis.pbix    # Full Power BI dashboard file
└── README.md
```
...
1. Download and install **Power BI Desktop** (free at powerbi.microsoft.com)
...
-- Income Utilization %
Income Utilization % =
DIVIDE(SUM('transactions'[spend_amount]), SUM('customers'[monthly_income])) * 100

-- Average Transaction Value
Avg Transaction Value =
AVERAGEX('transactions', 'transactions'[spend_amount])

-- Credit Card Usage Rate
CC Usage Rate =
DIVIDE(
    CALCULATE(COUNT('transactions'[id]), 'transactions'[payment_type] = "Credit Card"),
